...
import { getWeekDates, calculate } from '@/helpers/_index';
...
export const PromoDetails = () => {
  ...
  const share = async () => {
    try {
      ...
      await saveInCacheAndShare((mainDataSuccess && mainData.name) || 'Code', blob);
  ...
  const save = async () => {
    try {
      setDownloadingLoader(true);
      const blob = await downloadExcel(codeId);
      await saveInStorage((mainDataSuccess && mainData.name) || 'Code', blob);
    } catch (e: SerializedError | any) {
      setDownloadingLoader(false);
      Toast.show({ type: 'error', text1: e.message });
    }
          ...
          mainDataSuccess && (
            <>
              ...
              {mainDataSuccess && (
                <Statistics
                  graphData={
                    statisticsData && selectedDays.map((date) => statisticsData[date] || 0)
                  }
